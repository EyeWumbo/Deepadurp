...
public class CPUSchedulerFrame extends JFrame implements ActionListener {
    ...
    JCheckBox startCB;
    JTextField quantumField, memoryConstraintField;
	...
	memPanel = new MemoryPanel("Memory");
	memPanel.setStats(0,0);
	memPanel.setToolTipText("What with the what now");
	...
	/*else if( e.getSource() == priCB){
		...
		if( e.getSource() == algButtons.get(i)){
		    ...
		    try{
		    	MultiLevelPriorityAlgorithm mlpa = (MultiLevelPriorityAlgorithm)newAlg;
		    	mlpa.setQuantum(Integer.parseInt(quantumField.getText()));
		    }
	...
	turnSP.setStats( cpu.getMinTurn(),
			 ...
			 cpu.getMaxTurn(),
			 cpu.getStdDevTurn());
	memPanel.setStats(cpu.getCurrentMemoryUsage(), Integer.parseInt(memoryConstraintField.getText()));
    }   
	...
	memoryConstraintField = new JTextField("100", 10);
	memoryConstraintField.setToolTipText("Memory Limit");
